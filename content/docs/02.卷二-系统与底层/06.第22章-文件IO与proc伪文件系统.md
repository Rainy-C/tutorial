---
title: "第 22 章 · 文件 IO 与 /proc 伪文件系统"
...
`0660 & ~umask`。默认 umask 通常是 `022`，所以实际是 `0640`。
要精确控制权限，创建后调用 `chmod(path, 0660)`。
:::
...
优点：简单、快。
缺点：结构体一改，旧配置文件就全乱了。
本项目的解法：文件短于结构体时，缺失部分用默认值补（第 12 章讲过 ConfigManager 的做法）。
第 92 章会给更健壮的版本化方案。
:::
...
技巧：`strchr` 找到 `-`，把它改成 `\0`，`line` 就变成了纯起始地址字符串，
再 `strtoul(.., 16)` 转数字。**不用手写解析。**
...
→ 下一章：[第23章-进程间通信概览](07.第23章-进程间通信概览)　—— 知道 6 种主流 IPC 各自的代价和适用场合，并理解本项目为什么选"共享内存 + 轮询标志"而不是别的。
