...
- [ ] 能解释为什么 `call printf` 在编译时填不了地址
- [ ] 能说出 PLT（代码）和 GOT（数据）各自的作用
- [ ] 知道延迟绑定：第一次调用才解析，第二次直接用
- [ ] 知道 `DT_SYMTAB` / `DT_STRTAB` 是找符号的入口
- [ ] 会用 `readelf -d` 和 `readelf --dyn-syms` 查看依赖与动态符号
- [ ] 知道 `dlsym` 背后就是这套机制

→ 下一章：[第21章-系统调用](05.第21章-系统调用)　—— 理解用户态如何向内核请求服务，能直接用 `syscall()` 写一个 hello world。
