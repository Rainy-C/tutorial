---
title: "第 76 章 · /proc/pid/maps 与模块基址"
tags: [教程, 卷六, 内存, maps]
day: 49
aliases: [ch76]
---
...
- [ ] 会用 `sscanf` 解析 maps 每一行
- [ ] 知道权限字符 `r/w/x/p` 的含义
- [ ] 会实现"后缀匹配 + 前一个是 /"的模块查找 ★
- [ ] 理解 segment（段）与 section（节）的区别
- [ ] 会筛出可扫描区域并合并相邻
- [ ] **跑通了 memview，能看到模块基址** ★
- [ ] 知道 maps 会变，不能永久缓存

→ 下一章：[第77章-ELF解析找符号](05.第77章-ELF解析找符号)　—— 从模块基址出发，解析内存里的 ELF 结构，按名字找到函数/变量的地址。
