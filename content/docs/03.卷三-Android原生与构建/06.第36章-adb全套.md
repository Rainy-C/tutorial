...
和 [第02章-装好第一套工具链](../01.卷一-编程地基/02.第02章-装好第一套工具链.md) 里 `where gcc` 是同一个坑：`where` 属于 cmd。
PowerShell 里用 `Get-Command adb` 或 `where.exe adb`。
三个终端的完整对照表见 [附录R-绝对零基础前置知识](../11.附录/21.附录R-绝对零基础前置知识.md) 第 3.2 节。
:::
...
展开说明见 [附录R-绝对零基础前置知识](../11.附录/21.附录R-绝对零基础前置知识.md) 第 3.2 节。
:::
...
→ 下一章：[第37章-root-权限与SELinux](07.第37章-root-权限与SELinux.md)　—— 理解 Android 的三层权限关卡（UID / capability / SELinux），知道为什么"明明是 root 还是被拒"。
