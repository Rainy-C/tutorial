...
:::info[这些函数属于 C 标准库（`<stdio.h>`），C++ 里是 `<cstdio>`]
...
→ 下一章：[第12章-引用-const-auto](12.第12章-引用-const-auto)　—— 掌握现代 C++ 的三个书写习惯：用引用代替指针传参、到处加 `const`、用 `auto` 减少重复。
