...
| **需要写 `*p` 或 `p->`**——看起来不清爽
| **可以乱改指向**——`p = &something_else;` 悄悄地改，很难追踪
...
→ 下一章：[第13章-模板与STL容器](13.第13章-模板与STL容器)　—— 会读会写简单模板，会用 `vector`/`map`/`optional`/`span`/`string_view`。
