...
:::info[`std::string` vs `std::string_view`]
...
→ 下一章：[第14章-智能指针与生命周期](14.第14章-智能指针与生命周期)　—— 不再手写 `new`/`delete`，能用 `unique_ptr`/`shared_ptr` 表达清楚"谁拥有这块内存"。
