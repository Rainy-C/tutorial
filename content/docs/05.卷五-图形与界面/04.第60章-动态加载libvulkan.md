...
:::info[`vkGetInstanceProcAddr` / `vkGetDeviceProcAddr`]
这两个是 Vulkan 的"元函数"：
...
→ 下一章：[第61章-ImGui架构与DrawList](05.第61章-ImGui架构与DrawList)　—— 理解立即模式 UI 的原理，看懂 `ImDrawList` 里到底装了什么。
