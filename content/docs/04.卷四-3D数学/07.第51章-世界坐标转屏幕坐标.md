...
- [ ] 能说出 W2S 的三步：相机空间 → NDC → 像素
- [ ] 知道失败时为什么要填 NAN 而不是 0
- [ ] 知道 px/py 是**半宽半高**，用错会偏一半
- [ ] **编译运行了 w2s_demo，7 个用例结果符合预期** ★
- [ ] 验证了对称性（左右、上下）和透视（距离翻倍偏移减半）
- [ ] 知道这个 demo 可以直接换成真实数据源

→ 下一章：[第52章-深度-裁剪与剔除](08.第52章-深度-裁剪与剔除)　—— 掌握 5 种剔除手段，知道本项目每一处 `continue` 背后的原因。
