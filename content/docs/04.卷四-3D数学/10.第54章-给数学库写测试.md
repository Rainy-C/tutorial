...
void TestVec3() {
    TEST("Vec3 基础运算");
    CHECK_NEAR(Vec3{1,2,3} + Vec3{10,20,30}, Vec3{11,22,33}.Length());
    // 上面写法不对，改成逐分量检查
    ...
    CHECK_NEAR(cr.x, 0); CHECK_NEAR(cr.y, 0); CHECK_NEAR(cr.z, 1);  // 右手系
...
void TestQuat() {
    ...
    TEST("四元数矩阵是正交的（行列 ±1，行向量长度 1）");
    Quat q = QuatFromAxisAngle({1,2,3}, 37);
    Mat4 mm = QuatToMatrix(q, Vec3{}, Vec3{1,1,1});
    for (int i = 0; i < 3; i++) {
...
void TestMatrix() {
    ...
    CHECK_NEAR(p1.x, 10);    // T*S：平移不受缩放影响
    CHECK_NEAR(p2.x, 20);    // S*T：平移被缩放放大
...
- [ ] 写了 test_framework.h（TEST/CHECK/CHECK_NEAR）
- [ ] mathlib 的测试覆盖了向量/角度/旋转/四元数/矩阵/相机
- [ ] **所有测试通过** ★
- [ ] 故意改坏代码，确认测试能抓到（变异测试）
- [ ] 知道"修 bug 要加回归测试"
- [ ] 把测试接入了构建或本地脚本

→ 下一章：[第55章-浮点误差与数值稳定](11.第55章-浮点误差与数值稳定)　—— 知道误差从哪来、怎么累积、怎么防。
