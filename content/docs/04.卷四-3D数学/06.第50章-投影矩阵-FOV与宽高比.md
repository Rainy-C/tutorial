...
**定义 `t = tan(FOV/2)`**，它把"深度"换算成"屏幕半高"。

## 投影的核心公式

给定相机空间的一点，深度为 `w`（前方距离）：

| 屏幕坐标 | 公式 |
|---|---|
| 横向 | `screenX = (camY / w) / t × (屏幕半宽)` |
| 纵向 | `screenY = -(camZ / w) / t × (屏幕半高)` |
...
两行**都除以同一个 `t`**。这说明：本项目假设 FOV 是**水平 FOV**，
并且垂直方向通过"乘以 py（屏幕半高）"来独立缩放。
...
等价于各自独立设置焦距。这就是为什么 UI 里有两个独立的微调系数：
...
- [ ] 知道 `t = tan(FOV/2)` 的几何含义（深度 → 半屏高的换算）
- [ ] 理解"除以 w"是透视除法（近大远小）
- [ ] 知道为什么 `screenY` 是 `1 - ndc`（Y 轴翻转）
- [ ] 理解本项目用 `px`/`py` 分别做横纵缩放，等价于独立焦距
- [ ] 知道"自算矩阵"和"读引擎矩阵"两种方案的优劣
- [ ] 知道焦距微调系数存在的意义
- [ ] 跑通投影验证，6 项断言全过

→ 下一章：[第51章-世界坐标转屏幕坐标](07.第51章-世界坐标转屏幕坐标)　—— 完成总纲承诺的第 5 个成果：**`w2s_demo`**——一个能在电脑上离线验证 W2S 完整链路的程序。
