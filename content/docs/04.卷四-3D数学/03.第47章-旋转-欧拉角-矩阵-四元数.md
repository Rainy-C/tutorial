...
- [ ] 知道 UE4 的 Pitch/Yaw/Roll 分别绕哪个轴
- [ ] 知道万向节死锁是什么、什么时候会碰到
- [ ] 能读懂 `rotatorToMatrix`，知道矩阵三行是前/右/上
- [ ] 知道四元数 `(x,y,z,w)` 与"轴+角"的对应关系
- [ ] 会用 `atan2` 把方向转成角度
- [ ] 知道角度是周期的，差值必须归一化到 ±180
- [ ] 跑通了互转验证程序

→ 下一章：[第48章-局部空间与父子变换链](04.第48章-局部空间与父子变换链)　—— 理解骨骼层级和"局部矩阵 × 父级矩阵"的链式计算，这正是本项目 `GetBoneWorldPos` 在做的事。
