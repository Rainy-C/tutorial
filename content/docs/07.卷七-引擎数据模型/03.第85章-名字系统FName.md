...
- [ ] 理解为什么用"字符串表 + ID"
- [ ] 知道 FName 的位布局（高 16 位块号、低 16 位偏移）
- [ ] 知道 `header >> 6` 取长度、`& 1` 判宽字符
- [ ] 理解 `blockOff * 2` 的含义
- [ ] 知道长度必须检查上界 ★
- [ ] 会写 `AppendUtf8`（UTF-32 → UTF-8）
- [ ] 跑通了名字表扫描器，输出与真值一致

→ 下一章：[第86章-变换组件与骨骼层级](04.第86章-变换组件与骨骼层级)　—— 把第 48 章的骨骼数学接到真实数据上，并给 demo 引擎加上骨骼，完成端到端验证。
